...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    } else if (billType == "/yonbip/scm/arrivalorder/list" || billType.indexOf("arrivalorder") > -1) {
      //采购到货
      if (operation == 1) {
        billName = "采购到货单";
      } else {
    ...
    } else if (billType == "/yonbip/sd/voucherdelivery/list" || billType.indexOf("voucherdelivery") > -1) {
      //销售发货
      if (operation == 1) {
        billName = "销售发货单";
      } else {
    ...
    for (let i = 0; i < udiList.length; i++) {
      ...
      udiTrack[0].optDate = udiList[i].createTime;
      if (billName == "销售出库单") {
        udiTrack.orderDetailId = udiList[i].orderDetailId;
      }
