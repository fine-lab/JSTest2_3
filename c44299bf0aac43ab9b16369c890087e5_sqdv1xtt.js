    ...
    if (isGmp) {
      ...
      if (typeof gmpInfoArrayData.paramRes !== "undefined" && gmpInfoArrayData.paramRes.length > 0) {
        ...
        for (let i = 0; i < gmpInfoArray.length; i++) {
          orgIds.push(gmpInfoArray[i].org_id);
        }
        let indexOrg = orgIds.indexOf(request.inOrg);
        if (indexOrg != -1) {
          for (let i = 0; i < gmpInfoArray.length; i++) {
            if (gmpInfoArray[i].org_id == request.inOrg) {
              if (gmpInfoArray[i].isPurchaseControl != 1 && gmpInfoArray[i].isPurchaseControl != "1" && request.type == "采购订单") {
                ...
                supPreqLicenManage[request.inOrg] = gmpInfoArray[i];
              }
            }
          }
        } else {
