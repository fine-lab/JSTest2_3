...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let configfun = extrequire("GT32085AT31.backDefaultGroup.base");
    let config = configfun.execute(request);
    var access_token = getToken(config.config.appkey, config.config.appSecret, config.config.sandboxopenapiurl);
    //获取token方法
