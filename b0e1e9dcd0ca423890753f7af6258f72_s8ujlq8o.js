...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let whereStr = " where id in (";
    for (var i = 0; i < data.length; i++) {
      let item = data[i];
      if (!item.vendorId) {
        continue;
      }
      whereStr += item.vendorId + ",";
    }
    if (whereStr != " where id in (") {
      whereStr = whereStr.substring(0, whereStr.length - 1);
      whereStr += ")";
      // 查询token信息
      whereStr = whereStr.replace("where id in", "where vendorId in");
      let tokenSql = "select vendorId,appKey,appSecret,appTenantId from GT39745AT6.GT39745AT6.suplierApiMessage " + whereStr;
      ...
      data.forEach((item) => {
        let body = { code: item.po_code, isCode: false };
        if (tokenMap.get(item.vendorId).appKey && tokenMap.get(item.vendorId).appSecret && tokenMap.get(item.vendorId).appTenantId) {
          //使用-传参分别为： appkey, appsecrect
          let header = { appkey: tokenMap.get(item.vendorId).appKey, appsecret: tokenMap.get(item.vendorId).appSecret };
          let url = configParams.gatewayUrl + "/" + tokenMap.get(item.vendorId).appTenantId + "/product_ref/product_ref_01/delSupplierShippingBill";
          let apiResponse = ublinker("post", url, JSON.stringify(header), JSON.stringify(body));
        }
        updateIds.push({ id: item.id, submit_status: item.submit_status });
      });
    }
    // 更新主表状态，防止重复撤销
    var res = ObjectStore.updateBatch("GT37595AT2.GT37595AT2.shippingschedule", updateIds, "shippingschedule");
    return { res };
    //获取token方法
    function getToken(yourappkey, yourappsecrect) {
      ...
      var access_token;
      // 获取token的url
      const token_url = configParams.tokenUrl;
