...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (tenantId === "vwsgqatm") {
      return { type: "SL" };
    }
    const access_token = getToken("0f60936b699f4bf093923eaef5a07424", "0ff167852cff9d166d70607c92cab4e3ac665707");
    //获取token方法
    function getToken(yourappkey, yourappsecrect) {
      ...
      //加密算法------------------------------------------------------------------------------------------
      const sha256 = CryptoJS.HmacSHA256(secrectdata, appsecrect);
    ...
    const strResponse = postman(
      "post",
      "https://www.example.com/" + access_token,
      JSON.stringify(header),
      JSON.stringify(body)
    );
    const jsonObj = JSON.parse(strResponse || "{}");
    if (jsonObj.code == "200" && jsonObj.data && jsonObj.data.recordList && jsonObj.data.recordList.length) {
      const supp = jsonObj.data.recordList[0];
      return { type: "supp", code: supp.code, name: supp.name };
    }
