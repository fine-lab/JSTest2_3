...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (address == "yonbip_scm_storeprorecord_list" || address.indexOf("storeprorecord") > -1) {
      ...
      //销售出库单
      sql = "select id,id mainId,org.name orgName,code billCode,vouchdate vouchDate from st.salesout.SalesOut  where  1=1 ";
      if (baseOrgId != undefined && baseOrgId != "" && baseOrgId != null) {
        sql += " and org.id ='" + baseOrgId + "'";
      }
      ...
      //销售发货
      sql =
        "select id,id mainId,stockOrgId.name orgName,code billCode,vouchdate vouchDate from 			voucher.delivery.DeliveryVoucher  where  (statusCode = 'PARTOUTSTOCKED' or statusCode = 'DELIVERING')  ";
      if (baseOrgId != undefined && baseOrgId != "" && baseOrgId != null) {
