...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let sqlStr =
      "select b.guoJiaMingCheng,c.productName,* " + //,b.guoJiaMingCheng,b.guoJiaBianMa,b.haiGuanBianMa,b.guoJiaMingCheng_Eng,b.jianCheng2,b.jianCheng3,b.dianHuaQuHao,b.shiQu ";
    ...
    let dataDetail = res[0];
    if (dataDetail.verifystate == 2) {
      //审核态
      extrequire("GT3734AT5.ServiceFunc.logToDB").execute(null, JSON.stringify({ LogToDB: true, logModule: 9, description: "线索询盘-客户重传:" + businessId, reqt: "", resp: JSON.stringify(res) }));
      ...
      let updobj = { id: businessId, tongBuZhuangTai: tongBuZhuangTai, tongBuShiiJan: getNowDate(), shiBaiYuanYin: shiBaiYuanYin };
      let updres = ObjectStore.updateById("GT3734AT5.GT3734AT5.XunPanXSBill", updobj, "66c03e66");
      extrequire("GT3734AT5.ServiceFunc.logToDB").execute(
        null,
        JSON.stringify({ LogToDB: LogToDB, logModule: 1, description: "更新同步状态", reqt: JSON.stringify(updobj), resp: JSON.stringify(updres) })
      ); //调用领域内函数写日志
