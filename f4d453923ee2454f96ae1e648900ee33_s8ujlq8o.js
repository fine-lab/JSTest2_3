...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (var i = 0; i < billData.length; i++) {
      ...
      let updateItem = { id: billData[i].id, submit_status: "1" };
      updateArr.push(updateItem);
      for (var l = 0; l < subBills.length; l++) {
        let shippingscheduleb = subBills[l];
        if (shippingscheduleb.sended_quantitiy == "" || parseInt(shippingscheduleb.quantitiy) != parseInt(shippingscheduleb.sended_quantitiy)) {
          errMsg += billData[i].po_code + ",";
        }
      }
    }
    if (errMsg != "") {
      errMsg = errMsg.substring(0, errMsg.length - 1) + " 未发货完全！";
    ...
    for (let [key, value] of tokenMap) {
      ...
      let apiResponse = ublinker("post", url, JSON.stringify(header), JSON.stringify({ whereStr: whereStr, opeStatusCode: "1", opeStatusName: "已确认" }));
      rrr = apiResponse;
