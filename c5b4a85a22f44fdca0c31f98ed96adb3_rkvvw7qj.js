  ...
  execute(context, param) {
    ...
    if (iscallback == "1") {
      ...
      for (let irow = 0; irow < orderDetails.length; irow++) {
        ...
        let qty = Number(priceQty) * Number(datas.invPriceExchRate); //数量 计价换算率60
        datas.set("qty", qty + "");
        let subQty = Number(qty) / Number(datas.invExchRate); //销售数量  销售换算率60/20=3
        datas.set("subQty", subQty + "");
        ...
        //原币无税特殊优惠
        orderDetailsPrices.set("particularlyMoneyOrigTaxfree", datas.particularlyMoney + "");
        //原币无税促销
        orderDetailsPrices.set("promotionMoneyOrigTaxfree", "0");
        //原币无税抵扣积分
        orderDetailsPrices.set("pointsMoneyOrigTaxfree", "0");
        //报价本币含税金额
        orderDetailsPrices.set("saleCost_domestic", datas.saleCost + "");
        //本币含税金额
        let odpnatSum = Number(oriTaxUnitPrice) * Number(priceQty);
        orderDetailsPrices.set("natSum", odpnatSum + "");
        //本币含税单价
        orderDetailsPrices.set("natTaxUnitPrice", datas.natTaxUnitPrice + "");
        //本币分摊返利
        ...
        //计价数量
        orderDetailsPrices.set("priceQty", datas.priceQty + "");
        //数量
        let qtypric = Number(datas.priceQty) * Number(datas.invPriceExchRate);
        orderDetailsPrices.set("qty", qtypric + "");
        //销售数量
        ...
        //含税金额
        orderDetailsPrices.set("oriSum", datas.oriSum + "");
        //扣率
