    ...
    if (createRules != null && createRules.length != 0) {
      ...
      if (serialList != null && serialList != undefined && serialList.length > 0) {
        let releaseUdiSerial = ObjectStore.queryByYonQL(
          "select serialNumber  from I0P_UDI.I0P_UDI.UDIFilev3 where dr = 0 and UDI in (select udiCode from I0P_UDI.I0P_UDI.udi_release_data_infov3 where dr = 0 and udiConfigId ='" +
            configId +
            "' and sourceCode = '" +
            request.billCode +
            "' and sourceType= '" +
        ...
        if ((udiCodeList == null || udiCodeList.length == 0) && otherOrderUdi != null && otherOrderUdi.length > 0) {
          throw new Error("当前UDI码已经存在：" + JSON.stringify(otherOrderUdi));
        } else if (udiCodeList == null || udiCodeList.length == 0) {
          throw new Error("单据可生成UDI序列号已用完！");
        }
      } else {
        //自定义序列号
        ...
        if (firstnumpos == -1 && lastnumpos == -1 && createUdiNum > 1) {
