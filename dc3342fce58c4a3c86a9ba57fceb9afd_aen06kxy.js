...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let myConfig = configFun.execute(request);
    var access_token = getToken(myConfig.config.appKey, myConfig.config.appSecret, myConfig.config.baseUrl);
    return { access_token };
    function getTokenCustom(appKey, appSecret, baseUrl) {
      //设置返回的access_token
      var access_token;
      // 获取token的url
      ...
      //加密算法------------------------------------------------------------------------------------------
      var sha256 = HmacSHA256(secrectdata, appSecret);
      const base64 = Base64Decode(sha256);
      // 获取签名
    ...
    //获取Token
    function getToken(appKey, appSecret, baseUrl) {
      //设置返回的access_token
      var access_token;
      // 获取token的url
      const token_url = baseUrl + "/open-auth/selfAppAuth/getAccessToken";
      ...
      //加密算法------------------------------------------------------------------------------------------
      var sha256 = CryptoJS.HmacSHA256(secrectdata, appSecret);
