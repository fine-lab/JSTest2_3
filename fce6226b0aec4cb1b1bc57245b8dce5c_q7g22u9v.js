...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    // 获取需要使用的字段
    var { agentId, id } = param.data[0];
    //通过agentid查找销售订单详情
    let Sql = "select * from voucher.order.OrderDetail where orderId ='" + id + "'";
    let salesOrder = ObjectStore.queryByYonQL(Sql, "udinghuo")[0];
    // 红蓝字判断,数值大于0时为蓝字单据,否则为红色字单据
    var billNum = salesOrder.qty;
