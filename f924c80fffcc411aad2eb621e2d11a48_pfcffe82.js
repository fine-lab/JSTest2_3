    ...
    for (let i = 0; i < bills.length; i++) {
      ...
      if (needInsertPlans && needInsertPlans.length > 0) {
        ...
        if (rewriteItems && rewriteItems.length > 0) {
          let rewriteRes = ObjectStore.updateBatch("GT7139AT4.GT7139AT4.sy_busiorderitem", rewriteItems);
        }
      }
      if (needUpdateMonthly && needUpdateMonthly.length > 0) {
        let updateRes = ObjectStore.updateBatch("GT7139AT4.GT7139AT4.sy_monthlyplans", needUpdateMonthly);
      }
    }
    return {};
  }
