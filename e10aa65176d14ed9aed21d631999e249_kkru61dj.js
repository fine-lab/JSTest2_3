run = function (event) {
  ...
  let getProSkuLicInfo = function (materialId, sku, orgId) {
    return new Promise((resolve, reject) => {
      cb.rest.invokeFunction(
        "GT22176AT10.publicFunction.getProSkuLicInfo",
        {
          materialId: materialId,
          sku: sku,
          orgId: orgId
  ...
  viewModel.on("afterLoadData", function () {
    ...
    let rows = gridModelInfo.getRows();
    if (currentState == "add") {
      getStockStateByName("合格").then((res) => {
        for (let i = 0; i < rows.length; i++) {
          gridModelInfo.setCellValue(i, "qualifiedstate", res.id);
          gridModelInfo.setCellValue(i, "qualifiedstate_name", res.name);
        }
      });
      getStockStateByName("不合格").then((res) => {
        for (let i = 0; i < rows.length; i++) {
          gridModelInfo.setCellValue(i, "noqualifiedstate", res.id);
          gridModelInfo.setCellValue(i, "noqualifiedstate_name", res.name);
        }
      });
    }
  ...
  //复核员变化后，如果复核部门为空，那么带出复核部门，
  viewModel.get("checker_name").on("afterValueChange", function (data) {
    let dep_id = viewModel.get("checkdep").getValue();
    if (data.value != null && (dep_id == null || dep_id == undefined) && (data.oldValue == null || data.value.id != data.oldValue.id)) {
      viewModel.get("checkdep").setValue(data.value.dept_id);
      viewModel.get("checkdep_name").setValue(data.value.dept_id_name);
    }
  });
  //复核部门切换，复核员清空
  ...
  viewModel.on("beforePush", function (data) {
    ...
    if (data.params.cSvcUrl.indexOf("targetBillNo=fb4f91ab") > 0) {
      ...
      let request = {
        ...
        uri: "GT22176AT10.GT22176AT10.Sy01_quareview"
      };
      ...
      let request = {
        id: viewModel.get("id").getValue(),
        uri: "GT22176AT10.GT22176AT10.SY01_bad_drugv7"
      };
      promises.push(validateLowerState(apiUrl, request).then(handerMessage));
      let pushFlag = false;
      for (let i = 0; i < gridModelInfo.getRows().length; i++) {
  ...
  let validatePassword = function (passwordJson) {
    return new Promise(function (resolve) {
      cb.rest.invokeFunction(
        "GT22176AT10.publicFunction.validatePassword",
        {
