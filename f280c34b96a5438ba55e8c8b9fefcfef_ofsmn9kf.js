run = function (event) {
  ...
  viewModel.on("afterLoadData", function (args) {
    ...
    viewModel.on("modeChange", function (data) {
      ...
      cb.rest.invokeFunction("GT22176AT10.publicFunction.exWhRecheckInit", { invoiceOrg: invoiceOrg, productIds: productIds }, function (err, res) {
        if (err) {
          cb.utils.alert(err.message, "error");
          return false;
        } else {
          ...
          if (productInfors != undefined && productInfors.length > 0) {
            for (let i = 0; i < productInfors.length; i++) {
              for (let j = 0; j < rows.length; j++) {
                if (productInfors[i].material == rows[j].material && productInfors[i].doubleReview == "1") {
