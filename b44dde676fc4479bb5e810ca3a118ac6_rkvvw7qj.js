...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    var orderDetails = data.st_othinrecordlist;
    //声明一个数组，存放后台的字段
    var datalos;
    var temp = new Array();
      ...
      if (dataod._selected == true) {
        ...
        if (totalsummary.length > 0) {
          ...
          for (var i = 0; i < b; i++) {
            if (totalsummary[i] == dataod.headDefine.define3_name) {
              ...
              //在json对象中添加键值对数据
              //总数量
              materialClassifySumJSON.num = dataod.othInRecords_qty / 1000;
              materialClassifySumJSON.Original_number = dataod.code;
              materialClassifySumJSON.commodity = dataod.othInRecords_product;
              materialClassifySumJSON.commodity_name = dataod.othInRecords_product_cName;
              ...
              //总数量也要变
              titleArray[i].total_num = titleArray[i].total_num + dataod.othInRecords_qty / 1000;
        ...
        if (biaoji == 0) {
          ...
          var objectlist = {
            ...
            //总数量
            total_num: dataod.othInRecords_qty / 1000,
            //车牌号
    ...
    if (obj.code == 999) {
      throw new Error("id为" + temp + "的【其他入库单单】插入了【运费结算单】实体失败" + ",失败原因为:" + obj.message);
    }
    if (obj.code == 310083) {
      throw new Error("id为" + temp + "的【其他入库单单】插入了【运费结算单】实体失败" + ",失败原因为:" + obj.message);
    } else {
      throw new Error("已经将id为" + temp + "的【其他入库单单】插入了【运费结算单】实体");
    }
    return {};
  }
}
