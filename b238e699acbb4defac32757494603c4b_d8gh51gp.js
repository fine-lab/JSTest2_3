...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    //查询产品线
    var jiancefangfaId = selSyres[0].jiancefangfa != undefined ? selSyres[0].jiancefangfa : 0;
    if (jiancefangfaId != 0) {
      var jcffSql = "select * from bd.basedocdef.CustomerDocVO where id = '" + jiancefangfaId + "'";
      var jcffRes = ObjectStore.queryByYonQL(jcffSql, "ucfbasedoc");
      if (jcffRes.length == 0) {
        var err = "  -- 检测方法字段查询为空,请检查 --  ";
        throw new Error(err);
      }
      selSyres[0].jiancefangfa = jcffRes[0].name;
    }
    //业务员数据库查询
    ...
    //送检单位数据库查询
    if (selSyres[0].songjiandanwei != undefined) {
      var songjiandanweiid = selSyres[0].songjiandanwei;
      var songjiandanweisql = "select * from aa.merchant.Merchant where id = '" + songjiandanweiid + "'";
      var songjiandanweisqlRes = ObjectStore.queryByYonQL(songjiandanweisql, "productcenter");
      if (songjiandanweisqlRes.length == 0) {
        var err = "  -- 结算单位字段查询为空,请检查 --  ";
        throw new Error(err);
      }
      var songjiandanwei = songjiandanweisqlRes[0].name;
      selSyres[0].songjiandanwei = songjiandanwei;
    }
    //判断类型
    ...
    if (selSyres[0].soaType != undefined) {
      if (selSyres[0].soaType == "10") {
        var soaYB = "外周血";
      }
      selSyres[0].soaYB = soaYB;
    }
    //肿瘤样本类型
    if (selSyres[0].zlType != undefined) {
      if (selSyres[0].zlType == "10") {
        var ZLsYB = "外周血";
      } else if (selSyres[0].zlType == "20") {
        var ZLsYB = "外周血+石蜡切片";
      } else if (selSyres[0].zlType == "30") {
        var ZLsYB = "石蜡切片";
      } else if (selSyres[0].zlType == "40") {
        var ZLsYB = "组织";
      } else if (selSyres[0].zlType == "50") {
        var ZLsYB = "外周血+组织";
      } else if (selSyres[0].zlType == "60") {
        var ZLsYB = "蜡块";
      } else if (selSyres[0].zlType == "70") {
        var ZLsYB = "外周血+蜡块";
      } else if (selSyres[0].zlType == "80") {
        var ZLsYB = "子宫内膜脱落细胞";
      } else if (selSyres[0].zlType == "99") {
        var ZLsYB = "其他";
      }
      selSyres[0].ZLsYB = ZLsYB;
    }
    //  性别
    ...
    //性别
    if (selSyres[0].tubianjiandingshifuweibengongsijiancechanpin != undefined) {
      if (selSyres[0].tubianjiandingshifuweibengongsijiancechanpin == "true") {
        var sfcpmc = "是";
      } else if (selSyres[0].tubianjiandingshifuweibengongsijiancechanpin == "false") {
        var sfcpmc = "否";
      }
      selSyres[0].sfcpmc = sfcpmc;
    }
    //失眠
    ...
    //血液线样本类型
    if (selSyres[0].xyType != undefined) {
      if (selSyres[0].xyType == "10") {
        var xyType = "骨髓EDTA抗凝管(紫)";
      } else if (selSyres[0].xyType == "20") {
        var xyType = "骨髓肝素抗凝管(绿)";
      } else if (selSyres[0].xyType == "30") {
        var xyType = "外周血EDTA抗凝管(紫)";
      } else if (selSyres[0].xyType == "40") {
        var xyType = "外周血肝素抗凝管(绿)";
      } else if (selSyres[0].xyType == "50") {
        var xyType = "外周血游离DNA管";
      } else if (selSyres[0].xyType == "60") {
        var xyType = "蜡块";
      } else if (selSyres[0].xyType == "70") {
        var xyType = "切片";
      } else if (selSyres[0].xyType == "80") {
        var xyType = "骨髓活检物";
      } else if (selSyres[0].xyType == "90") {
        var xyType = "淋巴结活检物";
      } else if (selSyres[0].xyType == "100") {
        var xyType = "骨髓涂片";
      } else if (selSyres[0].xyType == "110") {
        var xyType = "血涂片";
      } else if (selSyres[0].xyType == "120") {
        var xyType = "脑脊液";
      } else if (selSyres[0].xyType == "130") {
        var xyType = "胸腹水";
      } else if (selSyres[0].xyType == "140") {
        var xyType = "DNA";
      } else if (selSyres[0].xyType == "150") {
        var xyType = "RNA";
      } else if (selSyres[0].xyType == "999") {
        var xyType = "其他";
      }
      selSyres[0].xyType = xyType;
    }
    //检测时期
    if (selSyres[0].jcsqType != undefined) {
      if (selSyres[0].jcsqType == "10") {
        var jcsqType = "初诊";
      } else if (selSyres[0].jcsqType == "20") {
        var jcsqType = "治疗后";
      } else if (selSyres[0].jcsqType == "30") {
        var jcsqType = "复发";
      } else if (selSyres[0].jcsqType == "99") {
        var jcsqType = "其他";
      }
      selSyres[0].jcsqType = jcsqType;
    }
    ...
    if (selSyres[0].dnaCPType != undefined) {
      if (selSyres[0].dnaCPType == "1") {
        var dnaCPName = "血浆";
      } else if (selSyres[0].dnaCPType == "2") {
        var dnaCPName = "脑脊液";
      } else if (selSyres[0].dnaCPType == "3") {
        var dnaCPName = "呼吸道样本";
      } else if (selSyres[0].dnaCPType == "4") {
        var dnaCPName = "局灶样本";
      }
      selSyres[0].dnaCPType = dnaCPName;
    }
    return { selSyres };
  }
}
