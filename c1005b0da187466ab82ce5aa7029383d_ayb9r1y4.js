        ...
        if (id == pid) {
          ...
          for (var q = 0; q < res2.length; q++) {
            ...
            if (sgzwid == zid) {
              ...
              // 判断技检报告天数大于58小于60时
              if (reportDateDays > 58 && reportDateDays < 60) {
                for (var z = 0; z < res3.length; z++) {
                  // 获取完工孙表外键
                  var wgwid = res3[z].BasicInformationDetails_id;
                  // 获取使用登记证办理日期
                  var a = res3[z].shiyongdengjizhengbanliriqi;
                  // 判断施工中主键是否等于完工外键
                  if (zid == wgwid) {
                    // 判断使用登记证办理日期
                    if (a == null) {
                      var YJType = "技检报告日期预警!";
                      var YJContent =
                        "请注意合同号：" + hth + "下的生产工号为：" + scgh + ",距您上一次添加技检报告日期已经过去了五十九天,离六十天仅剩一天,请您在六十天之前完成,请您尽快处理,祝您生活愉快再见！";
                      //调用预警公共函数
