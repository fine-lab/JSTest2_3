viewModel.on("afterLoadData", function (data) {
  setCommLogVisible("8578c6e5ad0249fd95dc7de38eb7d697");
  let tongBuZhuangTai = viewModel.get("tongBuZhuangTai").getValue();
  if (tongBuZhuangTai) {
    viewModel.get("shiBaiYuanYin").setState("cShowCaption", "富通客户ID");
    viewModel.get("button103tj").setVisible(true); //删除富通关联客户
    viewModel.get("button42ic").setVisible(true); //拉取信息进行同步
  } else {
    viewModel.get("shiBaiYuanYin").setState("cShowCaption", "失败原因");
    viewModel.get("button103tj").setVisible(false); //删除富通关联客户
    viewModel.get("button42ic").setVisible(false); //拉取信息进行同步
  }
  let isRelated = viewModel.get("isRelated").getValue(); //是否已生成系统档案
  viewModel.get("button72hh").setVisible(!tongBuZhuangTai); //重传
  let id = viewModel.get("id").getValue();
  if (id == undefined || id == "") {
    viewModel.get("button72hh").setVisible(false);
  }
...
viewModel.get("button72hh") &&
  viewModel.get("button72hh").on("click", function (data) {
    // 重传--单击
    let tongBuZhuangTai = viewModel.get("tongBuZhuangTai").getValue();
    if (tongBuZhuangTai) {
...
viewModel.get("button42ic") &&
  viewModel.get("button42ic").on("click", function (data) {
    // 拉取客户信息-从富通更新--单击
    ...
    debugger;
    cb.rest.invokeFunction("GT3734AT5.APIFunc.getCustFromFTApi", { custCode: code, custId: custId, orgName: orgName, billNo: viewModel.getParams().billNo }, function (err, res) {
      debugger;
      if (err == null) {
        var rst = res.rst;
        if (rst) {
...
viewModel.get("button103tj") &&
  viewModel.get("button103tj").on("click", function (data) {
    // 删除富通客户--单击
    let tongBuZhuangTai = viewModel.get("tongBuZhuangTai").getValue();
    if (!tongBuZhuangTai) {
