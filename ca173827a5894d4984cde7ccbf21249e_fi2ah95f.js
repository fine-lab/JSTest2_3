...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    //先判断是否是集团的人员
    var orgsql = "select StaffNew from GT36354AT20.GT36354AT20.org where StaffNew='" + userid + "'";
    var orgres = ObjectStore.queryByYonQL(orgsql);
    if (orgres.length > 0) {
      allData = "all";
    } else {
      //判断是否为管理人员 -----一个大区的区长默认为是有下属门店的，根据门店来获取区长信息
      var managerSql = "select StaffNew, id, area.StaffNew as areapsn,area.id as areaid from GT36354AT20.GT36354AT20.store where " + " StaffNew='" + userid + "' or area.StaffNew ='" + userid + "'";
