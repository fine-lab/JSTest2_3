...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    var gatewayUrl = "https://www.example.com/";
    // 查询销售日报子表数据
    ...
    // 客户表
    let custSql = "select * from aa.merchant.Merchant where code = '" + wareCode + "'";
    let custRes = ObjectStore.queryByYonQL(custSql, "productcenter");
    if (custRes.length == 0) {
      throw new Error("该仓库在客户档案中查询不到！");
    } else {
      // 客户交易关系
      let AgentRelation = "select * from aa.agent.AgentRelation where agentId ='" + custRes[0].id + "'";
      ...
      if (AgentRelationRes.length != 0) {
        ...
        if (taxRateId == "" || taxRateId == null) {
          ...
          for (let qt = 0; qt < Dlist.length; qt++) {
            ...
            // 获取计价单位
            let ProductSunData = "select * from pc.product.ProductDetail where productId = '" + pruId + "'";
            let ProductSunDataRes = ObjectStore.queryByYonQL(ProductSunData, "productcenter");
            if (ProductSunDataRes.length == 0) {
              // 计价单位
              let batchPriceUnit = "";
              let batchPriceUnitBool = includes(ProductSunDataRes, "batchPriceUnit");
              if (batchPriceUnitBool == false) {
                batchPriceUnit = ProductSunDataRes[0].batchPriceUnit;
              }
              ...
              if (pruDetailRes[0].isBatchManage != true) {
                ...
                // 含税金额
                let recordGradients_price_aounmt = price * count;
                let recordGradients_price_aounmt_count = Math.round(recordGradients_price_aounmt * 100) / 100;
                // 无税金额
                let NorecordGradients_price_aounmt = Money * count;
                ...
                let zbdata = {
                  salesOutsDefineCharacter: bodyDefineMap,
                  product: pruId,
                  productsku: sku,
                  unit: unitId,
                  stockUnitId: unitId,
                ...
                for (var i = 0; i < currentRes.length; i++) {
                  ...
                  if (batchRes.length == 0) {
                    ...
                    // 含税金额
                    let recordGradients_price = price * counts;
                    let recordGradients_price_count = Math.round(recordGradients_price * 100) / 100;
                    // 无税金额
                    let NorecordGradients_price = Money * counts;
          ...
          if (makeBodyObjs.length == 0) {
            ...
            let url = gatewayUrl + "/yonbip/scm/salesout/single/save?access_token=" + access_token;
            ...
            let auditurl = gatewayUrl + "/yonbip/scm/salesout/batchaudit?access_token=" + access_token;
