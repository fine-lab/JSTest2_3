viewModel.get("button36mi") &&
  viewModel.get("button36mi").on("click", function (data) {
    // 存货--单击
...
viewModel.get("button63lb") &&
  viewModel.get("button63lb").on("click", function (data) {
    // 发票--单击
    var rows = viewModel.getGridModel().getSelectedRows();
    if (rows.length == 0) {
      cb.utils.alert("请先选择数据!");
...
viewModel.get("button49ji") &&
  viewModel.get("button49ji").on("click", function (data) {
    // 客户--单击
    var rows = viewModel.getGridModel().getSelectedRows();
    if (rows.length == 0) {
      cb.utils.alert("请先选择数据!");
