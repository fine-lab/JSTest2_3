    ...
    function updateDate(type, detailsData) {
      ...
      if (dbckResbody.length > 0) {
        ...
        if (dbddResboy.length > 0) {
          ...
          //查询调拨订单子表自由自定义项数据
          if (transferApplys.transferApplysDefineCharacter.attrext11 != null) {
            //查询销售预订单子表
            let queryXSYDDbody = "select * from GT83441AT1.GT83441AT1.salesAdvanceOrder_b where id='" + transferApplys.transferApplysDefineCharacter.attrext11 + "'";
