//设置保存前校验
// 采购订单--单击
viewModel.get("button73ka") &&
  viewModel.get("button73ka").on("click", function (data) {
    // 获取单击行号
    let currentLine = data.index;
    // 当前点击的那一行
    let currentRowData = viewModel.get("materOuts").getRow(currentLine);
    // 当前行物料id
