function ChangeOfQualificationFile(event) {
  var viewModel = this;
  //原供应商经营范围(子表)
  var oldNOBModel = viewModel.get("XPH_OldNOBCCOrderList");
  //新供应商经营范围(子表)
  var newNOBModel = viewModel.get("XPH_NewNOBCCOrderList");
  ...
  function addNOBDatas() {
    var respResult = cb.rest.invokeFunction("0d412b7b08ea4727b393316c503eebdf", { enterpriseCode: enterpriseCode, typeCode: qualificationCode }, null, viewModel, { async: false });
    if (typeof respResult.error != "undefined") {
      clearHeadDatas();
      cb.utils.alert('根据"企业编码"和"类型编码"查无此供应商档案,请重新选择!');
      return false;
    }
