  ...
  execute(request) {
    ...
    if (request.OrderType == 1) {
      ...
      if (res == null || res.length == 0) {
        let object = { saleOrderCode: request.saleOrderCode, saleOrderId: request.saleOrderId, voucherJson: request.voucherJson, feeVoucherJson: request.feeVoucherJson, voucherTime: voucherTime };
        let insertRes = ObjectStore.insert("GT3734AT5.GT3734AT5.VoucherJson", object, "45f96d49");
      } else {
        //已有记录--修改
        let object = {
          id: res[0].id,
          saleOrderCode: request.saleOrderCode,
          saleOrderId: request.saleOrderId,
          voucherJson: request.voucherJson,
          feeVoucherJson: request.feeVoucherJson,
