...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    function savecosdoc(args, condition) {
      ...
      if (condition !== "工作经费" && condition !== undefined && condition !== null && condition !== "") {
      ...
      if (condition !== "工作经费" && condition !== undefined && condition !== null && condition !== "") {
        ...
        if (prt_dat.data.custdocdefid !== undefined) {
          delete prt_dat.data.custdocdefid;
        }
        request.body = prt_dat;
        let res3 = func3.execute(request).res;
        // 是否插入成功
        if (res3.code !== "200") {
          throw new Error("res3 =" + JSON.stringify(res3));
        }
