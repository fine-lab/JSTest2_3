...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if ("200" == responseObj.code) {
      ...
      if (price != -1) return { price: price, strResponse: JSON.parse(strResponse), type: "客户价格" };
      ...
      if ("200" == responseObj.code) {
        ...
        if (price != -1) {
          return { price: price, strResponse: JSON.parse(strResponse), type: "商品价格" };
        }
        return { strResponse: strResponse, price: price };
      }
    }
    return { strResponse: strResponse, "responseObj.code": "200" == responseObj.code };
  }
}
