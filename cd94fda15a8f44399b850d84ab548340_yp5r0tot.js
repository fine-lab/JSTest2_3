...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    if (request.appkey === undefined || request.appSecret === undefined) {
      return { access_token: "unkown app" };
    }
    //获取access_token ，注意修改参数 appkey，AppSecret
    var access_token = getToken(request.appkey, request.appSecret);
    //获取token方法
    function getToken(yourappkey, yourappsecret) {
      //设置返回的access_token
      var access_token;
      // 获取token的url
      const token_url = "https://www.example.com/";
      const appkey = yourappkey;
      const appsecrect = yourappsecret;
      // 当前时间戳
