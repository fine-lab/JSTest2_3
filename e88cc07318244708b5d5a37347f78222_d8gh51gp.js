...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (sydType == "1637163064320589833" || sydType == "1637162832390258697") {
      //临床免费(1637163064320589833)||科研免费(1585840799320899588)
      return { sydType };
    }
    //检测项目含税单价
    ...
    //查询部门
    let deptSql = "select code from bd.adminOrg.AdminOrgVO where id = '" + ygbmid + "'";
    var deptRes = ObjectStore.queryByYonQL(deptSql, "ucf-org-center");
    if (deptRes.length == 0) {
      throw new Error("获取部门为空");
    }
    ...
    for (var i = 0; i < borrow.length; i++) {
      ...
      //收样单类型
      if (sydType == "1637162909718020096" || sydType == "1637162755111780353") {
        //临床收费(1637162909718020096)、个人现金(1637162755111780353)
      ...
      } else if (sydType == "1637162987040014344") {
        //科研收费(1637162987040014344)
    ...
    for (var i = 0; i < loan.length; i++) {
      ...
      if (sydType == "1637162909718020096" || sydType == "1637162755111780353") {
        ...
        dai(description, kmbm, hbje, filedCodesrkh, valueCodesrkh, filedCodexm, valueCodexm);
      } else if (sydType == "1637162987040014344") {
        //科研收费(1637162987040014344)
        //贷 （主营业务收入-科研）22
        if (fenluNum == "20" && flsrkeyan == false) {
          ...
          dai(description, kmbm, fljcprojectwushuidanjia, filedCodesrkh, valueCodesrkh, filedCodexm, valueCodexm);
    ...
    function dai(description, kmbm, hbje, filedCodesrkh, valueCodesrkh, filedCodexm, valueCodexm) {
      srbodi = {
