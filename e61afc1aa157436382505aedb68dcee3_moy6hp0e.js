...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    try {
      ...
      if (define5Info.length > 0) {
        body.data.headFreeItem.define5 = define5Info[0].b_id; //发票地址
      }
      ...
      for (var i = 0; i < orderDetails.length; i++) {
        ...
        if (customerLevel) {
          //存在就计算
          let oriTaxUnitPrice = this.getOriTaxUnitPrice(customerLevel, productId); //含税单价没有打折
          let taxRate = 0; //税率  = "A7";
          if (Detail.taxId != "A7") {
          ...
          if (oriTaxUnitPrice > 0) {
            //含税价格大于0计算
            let Amountinfo = this.getAmount(Detail.qty, taxRate, body.data["orderPrices!exchRate"], oriTaxUnitPrice); //没有折扣后的价格
            //判断是否维护了津贴折扣
            //报价
            ...
            let discountoriTaxUnitPrice = oriTaxUnitPrice;
  ...
  getAmount(qty, taxRate, exchRate, oriTaxUnitPrice) {
    ...
    let res = { oriTaxUnitPrice, oriUnitPrice, oriSum, oriMoney, oriTax, natTaxUnitPrice, natUnitPrice, natSum, natMoney, natTax };
    return res;
  }
