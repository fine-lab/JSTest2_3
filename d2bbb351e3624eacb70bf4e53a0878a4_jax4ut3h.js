...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    const secrectdata = "appKey" + "7300373c8b2f43be8c57b20bb8d21cb7" + "timestamp" + timestamp;
    var sha256 = CryptoJS.HmacSHA256(secrectdata, "03633374f0c54286ae4fa2f29a87a15a");
    const base64 = Base64stringify(sha256);
    // 获取签名
    const signature = encodeURIComponent(base64);
    let URL = "https://www.example.com/" + timestamp + "&signature=" + signature + "";
    var strResponse = postman("get", URL, JSON.stringify(header1), null);
    //获取token
    let access_token;
    ...
    let body2 = {
      app_id: "youridHere",
      entry_id: "youridHere", //6375e1278327a60008c6ec14 核销单
      limit: 10
    };
    let apiResponse2 = apiman("post", url2, JSON.stringify(header2), JSON.stringify(body2));
    let obj = JSON.parse(res.apiResponse2).data.data[0];
    // 保存
    let header = {
      "Content-Type": "application/json;charset=UTF-8",
      // 就是appCode
    ...
    let apiResponse = apiman("post", url, JSON.stringify(header), JSON.stringify(obj));
    return { access_token };
  }
}
