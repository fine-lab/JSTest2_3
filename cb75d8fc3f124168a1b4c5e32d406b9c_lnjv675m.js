...
viewModel.get("orgregister_1730536675558293512") &&
  viewModel.get("orgregister_1730536675558293512").on("afterSetDataSource", function (data) {
    //企业产权登记--设置数据源后
    // 表格--设置数据源后
    ...
    rows.forEach((data) => {
      ...
      actions.forEach((action) => {
        ...
        if (data.verifystate == 1 || data.verifystate == 2 || data.verifystate == 3) {
          if (action.cItemName == "button40eg") {
