...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let apiResponse;
    var token = getToken("2892c389b7a2421aa1283a9ba6a6eae8", "abf0e10004fe47c2953a1fbaec978902");
    let data = param.data[0];
    let yixiang = data.yixiang;
    let customercode = data.code;
    let id = data.id; //ID
    let qiyekubianma = data.qiyekubianma; //企业库编码
    let qiyeku_name = data.qiyeku_name; //企业库
    let qiyeku = data.qiyekuid; //企业库ID
    let apiResponseqiye = postman("get", "https://www.example.com/" + token + "&id=" + qiyeku, null, null);
    var apiResponseqiyejson = JSON.parse(apiResponseqiye);
    //解析json
    var queryCodeqiye = apiResponseqiyejson.code;
    //判断请求结果是否正确
    if (queryCodeqiye !== "200") {
      throw new Error("错误" + apiResponseqiyejson.message + qiyeku);
    } else {
      var ccode = apiResponseqiyejson.data.code; //得到code值
      if (ccode === undefined && qiyekubianma !== undefined) {
        //如果没有被定义就新增保存
        let header = {
        ...
        if (queryCode1 !== "200") {
          ...
          var object = { id: id, qiyeku: qiyeku_id, qiyekubianma: customercode, qiyeku_name: yixiang };
          var retobj = ObjectStore.updateById("GT56476AT31.GT56476AT31.JJyixiangkehuxinxi", object);
        }
      }
    }
    if (qiyekubianma === undefined && qiyeku_name === undefined && qiyeku === undefined) {
    }
    //获取token方法
