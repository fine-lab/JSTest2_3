...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (datasAll.hasOwnProperty("样本编号*") != true) {
      ...
      throw new Error(err);
    } else if (datasAll.hasOwnProperty("检测项目*") != true) {
    ...
    var yzs = datasAll["孕周"];
    if (yzs != null) {
      var yzsNumber = typeof yzs;
      if (yzsNumber != "number") {
        throw new Error("  -- 孕周输入格式有误,请重新输入 --  ");
      }
    }
    var yts = datasAll["孕天"];
    if (yts != null) {
      var ytsNumber = typeof yts;
      if (ytsNumber != "number") {
        throw new Error("  -- 孕天输入格式有误,请重新输入 --  ");
      }
    }
    ...
    var testings = datasAll["检测项目*"]; //检测项目编码
