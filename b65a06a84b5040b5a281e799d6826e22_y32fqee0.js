  ...
  viewModel.get("button30wj").on("click", function (data) {
    ...
    if (indexArr.length > 0) {
      ...
      gridModel.getSelectedRowIndexes().forEach((item) => {
        ...
        if (formData.Confirm_status == 1) {
          errArr.push({ AdvanceArrivalNoticeNo: formData.AdvanceArrivalNoticeNo, enable: "确认" });
        }
      });
      if (errArr.length > 0) {
        var errMsg = "";
        for (var index = 0; index < errArr.length; index++) {
          errMsg += "预到货通知单号(ASN): " + errArr[index].AdvanceArrivalNoticeNo + "，单据已经属于" + errArr[index].enable + "状态\n";
        }
...
viewModel.get("button35pk") &&
  viewModel.get("button35pk").on("click", function (data) {
    ...
    if (indexArr.length > 0) {
      ...
      gridModel.getSelectedRowIndexes().forEach((item) => {
        var formData = viewModel.getGridModel().getRowsByIndexes(item)[0];
        if (formData.Confirm_status == 0) {
          errArr.push({ AdvanceArrivalNoticeNo: formData.AdvanceArrivalNoticeNo, enable: "取消" });
        }
      });
      if (errArr.length > 0) {
        var errMsg = "";
