...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let LicenseValidity = data.LicenseValidity;
    let IsEarlywarning = data.IsEarlywarning;
    let LicenseValidityDate = new Date(LicenseValidity).getTime();
    let NowDate = (LicenseValidityDate - date.getTime()) / (1000 * 3600 * 24);
    if (NowDate <= 30) {
      // 修改状态
