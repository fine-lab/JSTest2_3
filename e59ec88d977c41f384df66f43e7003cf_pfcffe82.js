...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let sql1 =
      "select basePrice from GT7239AT6.GT7239AT6.cmmssn_mar_basprc_b where product = " +
      materialId +
      " and  cmmssn_mar_basprc_bFk.org_id = " +
      org_id +
      " and cmmssn_mar_basprc_bFk.cmmssn_merchant = " +
      cmmssnMerchant;
    let basePrice = ObjectStore.queryByYonQL(sql1);
    let sql2 = "select price,isPriceContainsTax from GT7239AT6.GT7239AT6.salesPriceList_b where product =" + materialId + " and salesPriceList_bFk.org_id = " + org_id + " and merchant = " + merchant;
    let price = ObjectStore.queryByYonQL(sql2);
    var productRes = {},
      taxrateInfo = {};
    var access_token = getToken("39dfa78142a44b1892b0d0acb8f5bb0f", "ba2a2bded3a84844baa71fe5a3e59e00");
    let productUrl = `https://api.diwork.com/yonbip/digitalModel/product/detail?access_token=${access_token}&id=${materialId}&productApplyRangeId=${productApplyRangeId}`;
    var productRes = postman("get", productUrl, null, null);
    productRes = JSON.parse(productRes);
    if (productRes && productRes.data && productRes.data.id) {
      productRes = productRes.data;
      let taxrateUrl = `https://api.diwork.com/yonbip/digitalModel/taxrate/findById?access_token=${access_token}&id=${productRes.detail.outTaxrate}`;
      var taxrateJson = postman("get", taxrateUrl, null, null);
      var taxrateRes = JSON.parse(taxrateJson);
      if (taxrateRes && taxrateRes.data && taxrateRes.data.id) {
        taxrateRes = taxrateRes.data;
      }
      let unitUrl = `https://api.diwork.com/yonbip/digitalModel/unit/detail?access_token=${access_token}&id=${productRes.unit}`;
      var unitJson = postman("get", unitUrl, null, null);
